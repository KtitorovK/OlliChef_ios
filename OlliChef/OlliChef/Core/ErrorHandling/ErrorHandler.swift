...
nonisolated struct ErrorContext {
...
/// service should call this so it lands in Crashlytics with consistent shape. Plain
/// nonisolated function — Crashlytics' own APIs are documented thread-safe, and every
/// caller here is background/actor-isolated code reporting an error, not UI code.
nonisolated func handleError(_ error: Error, context: ErrorContext = ErrorContext()) {
